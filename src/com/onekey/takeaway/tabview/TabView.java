...
public class TabView extends RelativeLayout {
	 ...
	 */
	private int mTabViewGravity = Gravity.TOP;
	/**
	...
	private void initCustomAttr(int attr, TypedArray typedArray) {
		if (attr == R.styleable.TabView_tab_textViewSelColor) {
			...
			mTextViewUnSelColor = typedArray
					.getColor(attr, mTextViewUnSelColor);
		} else if (attr == R.styleable.TabView_tab_tabViewBackgroundColor) {
			mTabViewBackgroundColor = typedArray.getColor(attr,
					mTabViewBackgroundColor);
		} else if (attr == R.styleable.TabView_tab_tabViewHeight) {
			mTabViewHeight = typedArray.getDimensionPixelSize(attr,
					mTabViewHeight);
		} else if (attr == R.styleable.TabView_imageViewTextViewMargin) {
			mImageViewTextViewMargin = typedArray.getDimensionPixelSize(attr,
					mImageViewTextViewMargin);
		} else if (attr == R.styleable.TabView_tab_textViewSize) {
			mTextViewSize = typedArray.getDimensionPixelSize(attr,
					mTextViewSize);
		} else if (attr == R.styleable.TabView_tab_imageViewWidth) {
			mImageViewWidth = typedArray.getDimensionPixelSize(attr,
					mImageViewWidth);
		} else if (attr == R.styleable.TabView_tab_imageViewHeight) {
			mImageViewHeight = typedArray.getDimensionPixelSize(attr,
					mImageViewHeight);
			...
			mTabViewDefaultPosition = typedArray.getInteger(attr,
					mTabViewDefaultPosition);
	...
	private void initView(Context context) {
		...
		if (mTabViewGravity == Gravity.BOTTOM) {
			...
			fragmentContainerParams.addRule(RelativeLayout.ABOVE,
					R.id.tabview_id);
			...
			fragmentContainerParams.addRule(RelativeLayout.RIGHT_OF,
					R.id.tabview_id);
		} else if (mTabViewGravity == Gravity.TOP) {
			tabviewParams = new RelativeLayout.LayoutParams(RMP, mTabViewHeight);
			tabview.setOrientation(LinearLayout.HORIZONTAL);
			tabviewParams.addRule(RelativeLayout.ALIGN_PARENT_TOP);
			fragmentContainerParams.addRule(RelativeLayout.BELOW,
					R.id.tabview_id);
			...
			fragmentContainerParams.addRule(RelativeLayout.LEFT_OF,
					R.id.tabview_id);
		}
	...
	private void initTabChildView() {
		...
		unselectedIconList = new ArrayList<Integer>();
...
//			tabChild.addView(imageview);
			...
			tabChild.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View view) {
					...
					showOrHide();
					if (listener != null) {
						listener.onTabChildClick(currentPosition, imageview,
								textview);
					}
				}
			});
...
//				ImageView iv = (ImageView) tabChild.getChildAt(0);
				TextView tv = (TextView) tabChild.getChildAt(0);
//				iv.setImageResource(unselectedIconList.get(i));
	...
	public interface OnTabChildClickListener {
		void onTabChildClick(int position, ImageView imageView,
				TextView textView);
	}

	private void showOrHide() {
		if (currentTabIndex != index) {
			FragmentTransaction trx = mSupportFragmentManager
					.beginTransaction();
